...
if [[ ! -f "$pubkey" ]]
then echo "Couldn't find the ssh public key: $pubkey" && exit
fi
...
then
  ...
  echo "Enter existing sudo password for REMOTE machine (no echo)"
  echo -n "> "
  read -s password
  echo
fi
...
then scp -i $prvkey $HOME/.bash_aliases $user@$hostname:~/.bash_aliases
fi
